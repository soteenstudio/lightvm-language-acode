...
const restoreGlobalProperty = (
	property: "window" | "acode" | "document",
...
describe("plugin entry point", () => {
	test("registers, initializes, and unmounts the LightVM plugin", async () => {
		...
		const originalAcode = Object.getOwnPropertyDescriptor(globalThis, "acode");
		...
		try {
				...
				require: (name: string) => {
					...
					if (name === "@codemirror/view") {
						return {
							...
							ViewPlugin: {
								fromClass: (plugin: any, options?: any) => ({
									plugin,
									options,
								}),
							},
			...
			restoreGlobalProperty("acode", originalAcode);
